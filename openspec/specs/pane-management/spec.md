...
#### Scenario: 並び順の永続化・復元
- **WHEN** ペインの並び順が変化する(生成・クローズ・ドラッグ入れ替え・並び順リセットのいずれか)
- **THEN** 生存ペインのdisplayId配列を`localStorage`へ保存する
- **WHEN** amm を起動する
- **THEN** 保存済みdisplayId配列の順にペインを再生成し、グリッド整列を適用する(位置・サイズ・比率は保存しない。生成順序=並び順のみが永続化対象)
