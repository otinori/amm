...
`tools/list` は少なくとも `send_message` / `list_participants` / `peek_queue` / `pane/open` / `pane/close` / `pane/wait_state` の 6 ツールを含めなければならない (SHALL)。ゲートウェイが構成されている場合は外部サーバのツールを末尾に追加する。ツール名は `mdi/*` から `pane/*` へリネーム済み（経緯: UDR-amm-20260713T0447-98f、単一ウィンドウ内ペイン管理方式への移行に伴う）。
