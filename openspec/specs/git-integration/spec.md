...
## Requirements
### Requirement: ペイン個別クローズ時の Git ガード
ユーザーがペインを個別にクローズ操作した場合、amm は当該ペインの起動作業ディレクトリが Git リポジトリ内かどうかを判定し、リポジトリ内であれば未コミット・未プッシュの確認を行わなければならない (SHALL)。アプリ全体の一括終了ではこの個別ガードをスキップし、アプリケーション終了時の一括処理（後述）に委ねる。

#### Scenario: 個別クローズでリポジトリ内の変更がある場合
- **WHEN** ユーザーが Git リポジトリ内で起動されたペインを閉じようとする
- **THEN** amm はリポジトリルートを特定し、未コミット・未プッシュの確認フローを実行する
- **AND** 確認フローでユーザーがキャンセルを選んだ場合、クローズを中止する

#### Scenario: 作業ディレクトリが Git リポジトリ外
- **WHEN** ペインの起動ディレクトリが Git 管理下にない、またはディレクトリが存在しない
- **THEN** amm は確認ダイアログを一切表示せずにクローズを続行する

### Requirement: アプリケーション終了時の Git ガード一括処理
amm 全体を終了する際、amm は全ての未破棄ペインの起動作業ディレクトリを収集し、リポジトリ単位に重複排除（大文字小文字を区別しない）した上で、リポジトリごとに 1 回だけ未コミット・未プッシュ確認を行わなければならない (SHALL)。

#### Scenario: 複数のペインが同一リポジトリを指している
- **WHEN** 2 つ以上のペインが同じ Git リポジトリ内の異なるサブディレクトリで起動されている状態でアプリを終了する
