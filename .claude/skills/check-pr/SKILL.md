...
コミット一覧を取得し、以下の規約（AGENTS.md §3）に沿っているか確認する:
...
```bash
grep '"version"' src/apps/Amm/src-tauri/Cargo.toml src/apps/Amm/src-tauri/tauri.conf.json
```

両ファイルの値が一致していることを確認した上で、バージョン番号をユーザーに口頭確認: 「バージョンは `X.X.X` で合っていますか？」
